...
let toggles = { scanned:true, priority:false, star:false, jewish:false, recent:false };
let sortField = null;
let sortDir = 1;
...
function baseFilter(r) {
  ...
  if(toggles.scanned && !r.digitized) return false;
  if(typ && r.record_type!==typ) return false;
  if(dist && r.district!==dist) return false;
  if(arch && r.archive!==arch) return false;
  if(tagF && !(tg[r.citation_key]||[]).includes(tagF)) return false;
  if(toggles.jewish && r.jewish_content!=='confirmed') return false;
  if(toggles.star && !st[r.citation_key]) return false;
  if(toggles.priority && !(r.priority && r.archive!=='DAVtsO')) return false;
  if(toggles.recent && daysSince(r.added_at)>RECENT_DAYS) return false;
...
function renderColumnHeader() {
  ...
  return `<div class="colhead"><span class="ch chstar"></span>${spans}<span class="ch chscan">Scan</span></div>`;
}
...
function rowHTML(r) {
  ...
  const badges = [
    isPri ? '<span class="badge priority">&#x26A1;</span>' : '',
    ...
    hasNote ? '<span class="badge">&#x25CF;</span>' : '',
    isLargeFile(r) ? '<span class="badge warn" title="Large file">&#x26A0;</span>' : ''
  ].filter(Boolean).join(' ');
  ...
  return `
    <div class="rrow-main">
      ...
      <div class="rc-loc">
        <div class="rc-town">${esc(townDisplayName(r.town))} ${badges}</div>
        <div class="rc-cite mono">${esc(r.district)} &middot; ${esc(callNumber(r))}</div>
      </div>
      <div class="rc-type">${esc(r.record_type||'Record')}${r.subtype?`<span class="subtype">${esc(r.subtype)}</span>`:''}</div>
      <div class="rc-years mono">${esc(r.years||'')}</div>
      <div class="rc-pages mono">${r.page_count?r.page_count+'pp':'&mdash;'}</div>
      <div class="rc-size mono">${r.file_size_bytes?fmtSize(r.file_size_bytes):'&mdash;'}</div>
      <div class="rc-archive mono">${esc(r.archive||'')}</div>
      <div class="rc-scan">${r.digitized ? `<a class="openscan" href="${esc(firstUrl)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Open&#x2197;${multiCount>1?` (${multiCount})`:''}</a>` : '<span class="badge lead">lead</span>'}</div>
    </div>
...
function detailHTML(r) {
  ...
  const tagPicker = tagDefsList.map(t=>{
    ...
    return `<button class="tagchipbtn" style="${on?tagStyle(t):''}" onclick="toggleRecordTag('${key}','${esc(t).replace(/'/g,"&#39;")}');rerenderRow('${key}')">${on?'&#x2713; ':''}${esc(t)}</button>`;
  ...
  const applied2=recordTags()[key]||[];
  const tagChips = applied2.map(t=>`<span class="tag-chip" style="${tagStyle(t)}">${esc(t)}</span>`).join('');
  ...
  return `<div class="rdetail">
    <div>
      ...
      ${tagChips?`<div class="rtags" style="margin-top:8px">${tagChips}</div>`:''}
    </div>
...
function rerenderRow(key) {
  const el=document.querySelector(`.rrow[data-key="${cssEsc(key)}"]`);
  if(!el) return;
  el.innerHTML=rowHTML(R.find(r=>r.citation_key===key));
  if(expandedKey===key) {
...
function toggleExpand(r, el) {
  ...
  const prevEl=expandedKey ? document.querySelector(`.rrow[data-key="${cssEsc(expandedKey)}"]`) : null;
...
function renderTable(container) {
  ...
  if(!recs.length) {
    container.innerHTML=`<div class="emptystate"><h3>No records match</h3><p>Try clearing filters or turning off &ldquo;Scanned Only.&rdquo;</p></div>`;
    return;
  }
  applySort(recs);
  container.innerHTML=renderColumnHeader();
  recs.forEach(r=>container.appendChild(makeRowEl(r)));
}
...
function updateStatusLine() {
  ...
  const shown=R.filter(r=>baseFilter(r)).length;
  if(toggles.scanned) {
    el.innerHTML=`<span class="dot"></span> Showing <b>${shown}</b> of <b>${totalScanned}</b> scanned records &mdash; <button class="esc" onclick="setScannedOnly(false)">show all ${totalAll}, including the ${totalAll-totalScanned} without a scan yet</button>`;
  } else {
    el.innerHTML=`<span class="dot"></span> Showing <b>${shown}</b> of all <b>${totalAll}</b> records, including leads without a scan &mdash; <button class="esc" onclick="setScannedOnly(true)">show only the ${totalScanned} with a scan</button>`;
  }
}
...
function syncToggleUI() {
  document.getElementById('t-scanned').classList.toggle('on', toggles.scanned);
  document.getElementById('t-priority').classList.toggle('on', toggles.priority);
  document.getElementById('t-star').classList.toggle('on', toggles.star);
  document.getElementById('t-jewish').classList.toggle('on', toggles.jewish);
  document.getElementById('t-recent').classList.toggle('on', toggles.recent);
}
function resetAll() {
  ...
  toggles={scanned:true,priority:false,star:false,jewish:false,recent:false};
  syncToggleUI();
  sortField=null;
  sortDir=1;
  render();
...
function render() {
  updateStatusLine();
  renderTable(document.getElementById('main'));
}
...
function populateDistrictFilter() {
  ...
  const districts=[...new Set(R.map(r=>r.district))].filter(Boolean).sort();
  sel.innerHTML='<option value="">All districts</option>'+districts.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('');
}
function populateArchiveFilter() {
  const sel=document.getElementById('f-archive');
  const archives=[...new Set(R.map(r=>r.archive))].filter(Boolean).sort();
  sel.innerHTML='<option value="">All archives</option>'+archives.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('');
}
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && expandedKey) {
    const el=document.querySelector(`.rrow[data-key="${cssEsc(expandedKey)}"]`);
