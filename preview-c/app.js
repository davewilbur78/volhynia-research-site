...
// ---------------- state ----------------
// Variant C "Middle Ground": keeps the Town/District/Archive lens tabs from
// nextgen (still useful, still novel), but renders each group as dense
// table-ish rows (classic's density) instead of padded cards. No settings
// gear panel — Record Type/Tag/Year range sit in a visible filter bar, and
// "Scanned Only" is a pill alongside Priority/Jewish/My saved/Added recently.
...
// ---------------- table row (Variant A's density, reused for every lens) ----------------
function rowHTML(r) {
...
// ---------------- grouped rows (town / district lenses) ----------------
function renderGroupedList(container, groupKey) {
  ...
  keys.forEach(k=>{
    ...
    hdr.innerHTML=renderColumnHeader();
    container.appendChild(hdr.firstChild);
    groups[k].forEach(r=>container.appendChild(makeRowEl(r)));
  });
}
    ...
    card.addEventListener('click', (e)=>{
      ...
      if(isOpen && !rows.dataset.built) {
        ...
        if(!visible.length) {
          ...
          applySort(visible);
          rows.innerHTML='';
          rows.appendChild((()=>{const h=document.createElement('div');h.innerHTML=renderColumnHeader();return h.firstChild;})());
          visible.forEach(r=>rows.appendChild(makeRowEl(r)));
        }
      }
    });
...
function renderMyResearch(container) {
  ...
  container.innerHTML='';
  const hdr=document.createElement('div');
  hdr.className='myrhdr';
  hdr.innerHTML=`<h2 style="font-size:18px">My Research</h2>`;
  container.appendChild(hdr);
  const colhdr=document.createElement('div');
  colhdr.innerHTML=renderColumnHeader();
  container.appendChild(colhdr.firstChild);
  active.forEach(r=>container.appendChild(makeRowEl(r)));
}
