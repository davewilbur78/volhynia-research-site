...
let pillFilters = { jewish:false, star:false, priority:false, recent:false };
let settings = { showUnscanned:false, appearance:'auto' };
...
function baseFilter(r, opts) {
  ...
  if(!opts.ignoreScan && !settings.showUnscanned && !r.digitized) return false;
...
function renderGroupedList(container, groupKey) {
  ...
  if(!recs.length) {
    container.innerHTML=`<div class="emptystate"><h3>No records match</h3><p>Try clearing the search or turning on &ldquo;include records without a scan.&rdquo;</p></div>`;
    return;
  }
  applySort(recs);
  const groups={};
  ...
  codes.forEach(code=>{
    ...
    card.addEventListener('click', (e)=>{
      ...
      if(isOpen && !rows.dataset.built) {
        ...
        if(!visible.length) {
          rows.innerHTML=`<div style="font-size:12px;color:var(--muted)">Everything here is a lead only &mdash; turn on &ldquo;include records without a scan&rdquo; in settings to see it.</div>`;
        } else {
          applySort(visible);
          visible.forEach(r=>{
...
function updateStatusLine() {
  ...
  if(lens==='archive') {
    el.innerHTML=`<span class="dot"></span> Coverage across every archive &mdash; search and the pills above narrow this too. Card rows still respect &ldquo;include records without a scan.&rdquo;`;
    return;
  }
  if(settings.showUnscanned) {
    el.innerHTML=`<span class="dot"></span> Showing all <b>${totalAll}</b> records, including leads without a scan &mdash; <button class="esc" onclick="setShowUnscanned(false)">show only the ${totalScanned} with a scan</button>`;
  } else {
    el.innerHTML=`<span class="dot"></span> Showing records with a scan to view &mdash; this is the default. <b>${totalScanned}</b> scanned &mdash; <button class="esc" onclick="setShowUnscanned(true)">show all ${totalAll}, including the ${totalAll-totalScanned} without a scan yet</button>`;
  }
}
function setShowUnscanned(v) {
  settings.showUnscanned=v;
  saveSettings();
  syncGearUI();
  render();
}

// ---------------- lens / pills / gear wiring ----------------
function setLens(l) {
  lens=l;
...
document.addEventListener('click', (e)=>{
  ...
  if(!panel) return;
  if(panel.classList.contains('open') && !e.target.closest('#gearpanel') && !e.target.closest('#gearbtn')) {
...
function applyAppearance() {
  ...
  if(settings.appearance==='light') root.setAttribute('data-theme','light');
  else if(settings.appearance==='dark') root.setAttribute('data-theme','dark');
  else root.removeAttribute('data-theme');
}
function syncGearUI() {
  document.getElementById('sw-unscanned').classList.toggle('on', settings.showUnscanned);
  document.querySelectorAll('#gp-appearance button').forEach(b=>b.classList.toggle('on', b.dataset.mode===settings.appearance));
}
function saveSettings() { try { localStorage.setItem('v-settings', JSON.stringify(settings)); } catch(e) {} }
function loadSettings() {
  try {
    const saved=JSON.parse(localStorage.getItem('v-settings')||'{}');
...
function resetAll() {
  ...
  pillFilters={jewish:false,star:false,priority:false,recent:false};
